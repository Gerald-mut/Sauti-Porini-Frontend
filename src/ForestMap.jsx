...
import { AlertTriangle, Radio, MapPin, ShieldCheck, Send, X } from 'lucide-react';
...
const ForestMap = ({ satelliteData = [], ussdData = [], iotData = [] }) => {
  ...
  const handlePetition = (coords) => {
    ...
    window.open(`mailto:governor@kakamega.go.ke?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`);
  };
            ...
            <div className="bg-slate-900/95 backdrop-blur-md border border-slate-700 text-white rounded-xl overflow-hidden shadow-2xl min-w-[260px]">
              <div className="px-4 py-3 border-b border-slate-700 flex justify-between items-center bg-slate-800/50">
                <div className="flex items-center gap-2 font-bold uppercase tracking-wider text-xs">
                   {popupInfo.type === 'satellite' && <><AlertTriangle size={14} className="text-red-500"/> <span className="text-red-400">Sat Alert</span></>}
                   {popupInfo.type === 'ussd' && <><MapPin size={14} className="text-blue-500"/> <span className="text-blue-400">Citizen Report</span></>}
                   {popupInfo.type === 'iot' && <><Radio size={14} className="text-yellow-500"/> <span className="text-yellow-400">Audio Detect</span></>}
                </div>
                <button onClick={() => setPopupInfo(null)} className="text-slate-400 hover:text-white transition-colors">
                  <X size={16} />
                </button>
              </div>
              ...
              <div className="p-4 space-y-3">
                <div className="text-sm text-slate-300">
                  {popupInfo.confidence && (
                    <div className="flex justify-between">
                      ...
                      <span className="font-mono text-white">{popupInfo.confidence}</span>
                    </div>
                  )}
                  ...
                  {popupInfo.sound_type && (
                    <div className="flex items-center gap-2 text-yellow-200">
                      <span>🔊 Sound:</span>
                      <span className="uppercase font-bold">{popupInfo.sound_type}</span>
                    </div>
                  )}
                </div>
                ...
                {popupInfo.blockchain_proof && (
                  <div className="bg-green-900/20 border border-green-500/30 p-2.5 rounded-lg flex items-start gap-3 mt-2">
                    ...
                    <div>
                      <p className="text-xs font-bold text-green-400 uppercase">Blockchain Verified</p>
