...
import { fetchSatelliteAlerts, fetchUssdReports, fetchIotEvents } from './services/api';
...
function App() {
  ...
  const getModalTitle = () => {
    switch (selectedCategory) {
      case 'satellite': return 'Deforestation Alerts (GFW)';
      case 'iot': return 'Acoustic Sensor Events';
      case 'ussd': return 'Community Reports';
      default: return '';
    }
  };
    ...
    <div className="relative w-full h-screen bg-black overflow-hidden font-sans">
      ...
      <div className="absolute top-4 left-4 right-4 flex justify-between items-start pointer-events-none z-10">
        ...
        >
          ...
          <div>
            <h1 className="text-xl font-bold text-white tracking-wide">Sauti <span className="text-green-400">Porini</span></h1>
            <div className="flex items-center gap-2 text-xs text-gray-300">
              <span className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></span>
              Live Monitoring: Kakamega Forest
            </div>
          </div>
          <Menu className="text-white/50 ml-2 group-hover:text-white transition-colors" size={20} />
        </div>
              ...
              <tbody className="text-gray-200 text-sm">
                {data.satellite_alerts.map(alert => (
                  <tr key={alert.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                    <td className="p-3 font-mono text-gray-400">{alert.alert_date || 'N/A'}</td>
                    <td className="p-3 font-mono">{alert.lat}, {alert.lon}</td>
                    <td className="p-3">
              ...
              <tbody className="text-gray-200 text-sm">
                {data.iot_events.map(event => (
                  <tr key={event.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                    <td className="p-3 font-mono text-gray-400">{event.detected_at ? new Date(event.detected_at).toLocaleTimeString() : 'N/A'}</td>
                    <td className="p-3 font-mono text-yellow-200">{event.sensor_id}</td>
                    <td className="p-3 uppercase font-bold text-yellow-400">{event.sound_type}</td>
                    <td className="p-3 font-mono">{event.lat}, {event.lon}</td>
                  </tr>
                ))}
              </tbody>
            </table>
              ...
              <tbody className="text-gray-200 text-sm">
                {data.ussd_reports.map(report => (
                  <tr key={report.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                    <td className="p-3 font-mono text-gray-400">{report.received_at ? new Date(report.received_at).toLocaleString() : 'N/A'}</td>
                    <td className="p-3 font-mono text-blue-300">{report.phone_number}</td>
                    <td className="p-3">
                      <div>{report.report_details}</div>
                      <div className="text-xs text-gray-500">{report.location_text}</div>
                    </td>
                    <td className="p-3">
                      {report.blockchain_proof ? (
                        <div className="flex items-center gap-2 bg-green-900/20 border border-green-500/20 px-2 py-1 rounded cursor-help" title={report.blockchain_proof}>
